...
//     // attr("area-id")获取属性area-id的值
//     // attr("area-id", "1")添加属性area-id并设置值为1
...
//     };
//
//     $.get("/api/v1.0/houses", params, function(data){
...
//                 if(action == "renew"){
//                     cur_page = 1;
//                     $(".house-list").html(template("house-list-tmpl",{houses:data.data.houses}));
//                 }else{
//                     cur_page = next_page;
//                     $(".house-list").append(template("house-list-tmpl",{house:data.data.houses}));
//                 }
//             }
//         }
//     })
...
//         if (data.errnum == "0"){
//             // 用户从首页标准到这个搜索页面是可能选择了城区, 所以尝试从url的查询字符串参数中提取用户选择的城区
//             var areaId = queryData["aid"];
//             // 如果提取到城区id的数据
//             if(areaId){
//                 // 遍历从后端获取到的城区信息, 添加到页面中
//                 for(var i=0; i<data.data.length; i++){
//                     // 对于url查询字符串参数中拿到的城区信息, 在页面中高亮展示
//                     // 后端获取到城区id是整形, 从url参数中获取到的是字符串类型, 所以将url参数中获取到的转换为整形
...
//                 }
//             }else{
//                 // 如果url参数中没有城区信息, 不需要做额外的处理, 直接遍历展示到页面中
//                 for (var i=0; i<data.data.length; i++){
//                     $(".filter-area").append('<li area-id="'+ data.data[i].aid +'">'+ data.data[i].aname +'</li>');
//                 }
//             }
//
//             // 页面添加好城区选项信息后, 更新展示房屋列表信息
        ...
        if (data.errnum == "0"){
            if (data.data.total_page == 0){
                ...
                if (action == "renew"){
                    ...
                    cur_page = next_page;
                    $(".house-list").append(template("house-list-tmpl",{houses:data.data.houses}));
                }
            }
        }
    })
...
$(document).ready(function(){
    ...
    $.get("/api/v1.0/areas", function (data) {
        if(data.errnum == "0"){
            // 用户从首页跳转到这个搜索页面是可能选择了城区, 所以尝试从url的查询字符串参数中提取用户选择的城区
            var areaId = queryData["aid"];
            // 如果提取到城区id的数据
            if(areaId){
                // 遍历从后端获取到的城区信息, 添加到页面中
                for(var i=0; i<data.data.length; i++){
                    // 对于url查询字符串参数中拿到的城区, 在页面中做高亮展示
                    // 后端获取到城区id是整型, 从url参数中获取到的是字符串型, 所以将url参数中获取到的转换为整型
                    areaId = parseInt(areaId);
                    if(data.data[i].aid == areaId){
                        $(".filter-area").append('<li area-id="'+ data.data[i].aid +'"class="active">'+data.data[i].aname+'</li>')
                    }else{
