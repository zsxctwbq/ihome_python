...
from . import db
from constants import QINIU_URL_FILELD, HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS
...
class User(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            "mobile": self.mobile,
            "avatar": QINIU_URL_FILELD + self.avatar_url if self.avatar_url else "",
...
class House(BaseModel, db.Model):
    ...
    #         # 房屋主图片的路径
    #         "image_url": QINIU_URL_FILELD + self.index_image_url if self.index_image_url else "",
    #         # 房间数目
    ...
    #         # 用户头像
    #         "user_avatar": QINIU_URL_FILELD + self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_full_dict(self):
        ...
        for img_url in self.images:
            img_urls.append(QINIU_URL_FILELD + img_url.url if img_url.url else "")
        ...
        try:
            orders = Order.query.filter(Order.house_id==self.id, Order.status=="COMPLETE", Order.comment!=None)\
                .order_by(Order.update_time.desc()).limit(HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
            ...
            for order in orders:
                comment = {
                    ...
                    "ctime": order.ureate_time.strftime("%Y-%m-%d")
                }
...
class Order(BaseModel, db.Model):
    ...
    trade_no = db.Column(db.String(80))  # 支付宝交易号
    ...
    def to_dict(self):
        ...
        order_dict = {
            ...
            "image_url": QINIU_URL_FILELD + self.house.index_image_url if self.house.index_image_url else "",
